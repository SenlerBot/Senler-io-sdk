"use strict";
/* tslint:disable */
/* eslint-disable */
/**
 * Ai Senler API
 * API . : API- senler_sk_... OAuth 2.0 Bearer-.
 *
 * The version of the OpenAPI document: 1.0
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.LandingMaxMiniAppResponseDtoTargetFromJSON = LandingMaxMiniAppResponseDtoTargetFromJSON;
exports.LandingMaxMiniAppResponseDtoTargetFromJSONTyped = LandingMaxMiniAppResponseDtoTargetFromJSONTyped;
exports.LandingMaxMiniAppResponseDtoTargetToJSON = LandingMaxMiniAppResponseDtoTargetToJSON;
exports.LandingMaxMiniAppResponseDtoTargetToJSONTyped = LandingMaxMiniAppResponseDtoTargetToJSONTyped;
const LandingCatalogLaunchTargetDto_1 = require("./LandingCatalogLaunchTargetDto");
const LandingSingleLaunchTargetDto_1 = require("./LandingSingleLaunchTargetDto");
function LandingMaxMiniAppResponseDtoTargetFromJSON(json) {
    return LandingMaxMiniAppResponseDtoTargetFromJSONTyped(json, false);
}
function LandingMaxMiniAppResponseDtoTargetFromJSONTyped(json, ignoreDiscriminator) {
    if (json == null) {
        return json;
    }
    switch (json['mode']) {
        case 'catalog':
            return Object.assign({}, (0, LandingCatalogLaunchTargetDto_1.LandingCatalogLaunchTargetDtoFromJSONTyped)(json, true), { mode: 'catalog' });
        case 'landing':
            return Object.assign({}, (0, LandingSingleLaunchTargetDto_1.LandingSingleLaunchTargetDtoFromJSONTyped)(json, true), { mode: 'landing' });
        default:
            throw new Error(`No variant of LandingMaxMiniAppResponseDtoTarget exists with 'mode=${json['mode']}'`);
    }
}
function LandingMaxMiniAppResponseDtoTargetToJSON(json) {
    return LandingMaxMiniAppResponseDtoTargetToJSONTyped(json, false);
}
function LandingMaxMiniAppResponseDtoTargetToJSONTyped(value, ignoreDiscriminator = false) {
    if (value == null) {
        return value;
    }
    switch (value['mode']) {
        case 'catalog':
            return Object.assign({}, (0, LandingCatalogLaunchTargetDto_1.LandingCatalogLaunchTargetDtoToJSON)(value), { mode: 'catalog' });
        case 'landing':
            return Object.assign({}, (0, LandingSingleLaunchTargetDto_1.LandingSingleLaunchTargetDtoToJSON)(value), { mode: 'landing' });
        default:
            throw new Error(`No variant of LandingMaxMiniAppResponseDtoTarget exists with 'mode=${value['mode']}'`);
    }
}
